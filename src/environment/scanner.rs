...
use super::{engine::Engine, utils::index_to_ivec3_bounds};
...
pub fn scan_mesh_unload(mut scanners: Query<&mut Scanner>, mut voxel_engine: ResMut<Engine>) {
    // find all loaded and check if in range
...
pub fn scan_mesh(mut scanners: Query<&mut Scanner>, mut voxel_engine: ResMut<Engine>) {
    for mut scanner in scanners.iter_mut() {
